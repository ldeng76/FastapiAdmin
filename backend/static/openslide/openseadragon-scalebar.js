...
(function($) {
    ...
    $.Scalebar.prototype = {
        ...
        drawMicroscopyScalebar: function(size, text) {
            ...
            // 横线在顶部，文字在横线下方显示（对齐病理查看器常用风格）
            this.divElt.style.borderTop = this.barThickness + "px solid " + this.color;
            this.divElt.style.backgroundColor = this.backgroundColor;
            // 文字与横线之间留一点间距
            this.divElt.style.paddingTop = "2px";
        ...
        getAsCanvas: function() {
            ...
            // MICROSCOPY 模式：横线画在顶部，文字画在下方
            if (this.drawScalebar === this.drawMicroscopyScalebar) {
                context.fillRect(0, 0, canvas.width, this.barThickness);
            } else {
            ...
            // 文字垂直对齐方式：MICROSCOPY 时靠下（横线在顶），其他居中
            context.textBaseline = this.drawScalebar === this.drawMicroscopyScalebar
                    ? "bottom" : "middle";
            context.fillStyle = this.fontColor;
            var hCenter = canvas.width / 2;
            var vPos = this.drawScalebar === this.drawMicroscopyScalebar
                    ? canvas.height - 2
                    : canvas.height / 2;
            context.fillText(this.divElt.textContent, hCenter, vPos);
            return canvas;
        }
    };
