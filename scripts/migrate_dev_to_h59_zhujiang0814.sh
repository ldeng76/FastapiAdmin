...
# 范围：仅珠江 0814 系列导入（0814 patient / 全量 CT / CT 正文修复重跑，
#       共 3 个 batch，见 BATCH_IDS）涉及的表与行，其他表/行一律不动：
#   - lnrs.lnrs_anon_ingest_batch   本次批次行
#   - lnrs.lnrs_anon_patient        center_code='zhujiang' 全部（新增+刷新）
#   - lnrs.lnrs_anon_exam           zhujiang CT（exam_type='CT'）
#   - lnrs.lnrs_anon_report_text    上述 CT exam 对应正文
#   - lnrs.lnrs_anon_exam_detail    created_batch_id ∈ 批次列表
#   - lnrs.lnrs_anon_phi_audit      batch_id ∈ 批次列表（先删后插，保 audit_id 对齐）
#   - lnrs.med_dict_mapping         zhujiang 医院全部映射（0011 种子新增 29 行）
#
...
#
# 使用方法（Git Bash，本机执行）：
#   ./scripts/migrate_dev_to_h59_zhujiang0814.sh
...
# ---- 批次定义（本次导入足迹） ----------------------------------------------
# 珠江 0814 系列共 3 个 batch：0814 patient / 全量 CT 首导 / CT 正文修复重跑。
...
BATCH_IDS="${BATCH_IDS:-2f5e131c-0e0c-4d49-806b-c1b5e6c5f4f8,3a2baddf-0961-4555-b8e6-1bcab52f1743,d65bff4e-ff72-4ef9-986c-e58bcfd821f5}"
