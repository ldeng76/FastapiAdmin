...
async def list_dicom_studies_controller(
    ...
    """Study 列表。"""
    result = await DicomService.list_studies_service(auth=auth)
...
async def list_dicom_series_controller(
    ...
    """Series 列表。"""
    result = await DicomService.list_series_service(auth=auth, study_id=study_id)
...
async def list_dicom_instances_controller(
    ...
    """切片列表（已排序）。"""
    result = await DicomService.list_instances_service(auth=auth, series_uid=series_uid)
...
async def get_dicom_instance_controller(
    ...
    """
    path = await DicomService.get_instance_path_service(auth=auth, sop_uid=sop_uid)
