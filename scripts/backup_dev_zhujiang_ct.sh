...
cat "$BACKUP_DIR/phi_audit_backup_batches.txt" | head -5 | awk '{print "       样例 batch:", $0}'

# ---- Step 2：构造 export.sql（4 张表 → 4 个 CSV）-----------------------
EXPORT_SQL="$BACKUP_DIR/export.sql"
cat > "$EXPORT_SQL" <<'EOF'
\set ON_ERROR_STOP on
EOF

# 2.1 report_text（按 exam JOIN 过滤）
cat >> "$EXPORT_SQL" <<EOF
\copy (
  SELECT rt.*
...
) TO '$(cygpath -m "$BACKUP_DIR")/report_text.csv' WITH (FORMAT csv, HEADER true)
EOF
...
) TO '$(cygpath -m "$BACKUP_DIR")/exam_detail.csv' WITH (FORMAT csv, HEADER true)
EOF
...
\copy (
  SELECT pa.*
  FROM lnrs.lnrs_anon_phi_audit pa
  WHERE pa.batch_id IN ('$(echo $BATCHES_INLINE | sed "s/,/','/g")')
) TO '$(cygpath -m "$BACKUP_DIR")/phi_audit.csv' WITH (FORMAT csv, HEADER true)
EOF
...
\copy (
  ...
  WHERE center_code = '$CENTER'
) TO '$(cygpath -m "$BACKUP_DIR")/patient.csv' WITH (FORMAT csv, HEADER true)
EOF

echo "=== Step 2: 导出 4 张表到 CSV ==="
psql_local -f "$EXPORT_SQL"
ls -lh "$BACKUP_DIR"/*.csv | awk '{printf "       %-40s %s\n", $9, $5}'

# ---- Step 3：行数 + md5 验证 ----------------------------------------------
echo "=== Step 3: 验证（CSV 行数 vs 库内 count(*)） ==="
# CSV 行数（HEADER 减 1；wc -l 在 Windows CRLF 上多 1 行，靠减 1 抵消）
RT_CSV=$(($(wc -l < "$BACKUP_DIR/report_text.csv") - 1))
ED_CSV=$(($(wc -l < "$BACKUP_DIR/exam_detail.csv") - 1))
PA_CSV=$(($(wc -l < "$BACKUP_DIR/phi_audit.csv") - 1))
PT_CSV=$(($(wc -l < "$BACKUP_DIR/patient.csv") - 1))
